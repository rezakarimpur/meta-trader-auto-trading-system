...
class LSTM:
    ...
    def __init__(self, file_path):
        ...
        self.model = load_model(os.path.join(os.path.dirname(__file__), 'my_model.keras'))
    ...
    def determine_signal(self, dframe):
        ...
        signal_prediction = self.model.predict(dframe)
        
        signal = int(np.round(signal_prediction[0][0]))
        
        print(f"Predicted Signal for the last candle: {signal}")
        return signal, self.df['Close'].iloc[-1] 
      
      
        
    def run_Lstm(self):
        recent_data= self.calculate_features()
        recent_data = recent_data.iloc[-170:]
        scaler=self.scaler
        recent_data_scaled = scaler.fit_transform(recent_data.values)
        recent_data_lstm = np.array([recent_data_scaled[-60:]])
        
        recent_data_lstm = np.nan_to_num(recent_data_lstm, nan=0.0)
        signal = self.determine_signal(recent_data_lstm)
        """
        to make it ready for output.py
        """
